...
- Kindle pricing tracked via [eReaderIQ](https://www.ereaderiq.com). Use ~~strikethrough~~ for original price when discounted.
- **The alert sits $0.01 below a reference price. Which price is the reference is the actual decision.**
  - *Current price* is the default — you'd take this book at today's price, so any drop at all should ping you. Most entries are this: `$2.99 🔔 $2.98`.
  - *A floor you missed* is the reference when eReaderIQ's price history shows the book has been lower and you didn't catch it. You're not accepting today's price; you're waiting for that floor to come back. `The Lies of Locke Lamora` is `$3.99 🔔 $1.98` — a cent under a $1.99 floor, not a cent under $3.99. `The Road` is `$7.99 🔔 $2.98` against a $2.99 floor.
  - The consequence worth understanding: a floor-referenced alert will stay silent through real sales. Locke Lamora dropping to $2.49 won't ping. That's the intent — but it means the alert is a *judgment about what this book is worth to you*, not a discount notifier, and it's worth re-checking when a book has sat in the queue a long time.
